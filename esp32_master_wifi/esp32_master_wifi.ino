...
// ─────────────────────────────────────────
//  HANDLER: TERIMA LOG DARI KAMERA (/log)
// ─────────────────────────────────────────
void handleLog() {
  ...
  if (camId < 1 || camId > 2) {
    server.send(400, "text/plain", "Invalid cam ID");
    return;
  }
...
void handleHeartbeat() {
  ...
  if (deserializeJson(doc, body) == DeserializationError::Ok) {
    ...
    String ip = doc["ip"] | "--";
    if (camId >= 1 && camId <= 2) {
      kamera[camId-1].lastUpdate = millis();
      kamera[camId-1].online     = true;
      if (ip != "--") {
        kamera[camId-1].ip = ip;
        if (camId == 1) IP_CAM1 = ip;
        if (camId == 2) IP_CAM2 = ip;
      }
      Serial.printf("[MASTER] ♥ HB dari CAM#%d\n", camId);
    }
  }
  server.send(200, "text/plain", "OK");
}
...
void setup() {
  ...
  // Terima log dari kamera via HTTP POST
  server.on("/log", HTTP_POST, [](){
    ...
    if (deserializeJson(doc, body) == DeserializationError::Ok) {
      ...
      if (camId >= 1 && camId <= 2) {
        ...
        statusSistem = tentukanStatus();
        Serial.printf("[MASTER] ← CAM#%d: %d hama | Δ+%d | %s\n",
  ...
  // Terima heartbeat dari kamera
  server.on("/hb", HTTP_GET, [](){
    int camId = server.arg("cam").toInt();
    if (camId >= 1 && camId <= 2) {
      kamera[camId-1].lastUpdate = millis();
      kamera[camId-1].online     = true;
      Serial.printf("[MASTER] ♥ Heartbeat CAM#%d\n", camId);
    }
    server.send(200, "text/plain", "OK");
  });
...
void loop() {
  ...
  // Data kamera diterima via HTTP POST /log (lihat route di setup)
